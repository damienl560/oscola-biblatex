% This file documents the biblatex-oscola package. It is (c) Paul Stanley 2014-2017; with further edits carried out by Michael Veale in 2026 for OSCOLA 5; both contributions are
% licensed under the Creative Commons Attribution 3.0-Unported License (CC BY 3.0). A copy of that
...
\author{Paul Stanley\thanks{pstanley@essexcourt.net}\\ Updated for OSCOLA 5 by Michael Veale\thanks{Professor of Technology Law and Policy, UCL, m.veale@ucl.ac.uk}}
...
legal citations set out in the fifth edition of
...
\index[general]{scope of package!international materials} The 4th edition of \oscolashort\ does not contain rules for formatting
...
contained extensive rules,\footcite[25--37]{oscola3} and the 5th edition restores and extends those rules.\footcite[45--62]{oscola5} The \oscolashort\
...
\item[ecli]\index[general]{ECLI numbers}
  ...
  whether the EU's ECLI numbers get printed. Under \oscolashort\ 5, the
  default is \verb|only|, in which case only the ECLI number is used.
  The alternative options are: \verb|add|, which prints the ECLI
  number in addition to any official report; and \verb|no|, in which
  case ECLI numbers are never printed. Note that if the case is
  otherwise unreported, ECLI numbers will always be used if
  available.
...
\item[\texttt{ibid}] \index[general]{customization!ibid}
  \index[general]{options!ibid@\texttt{ibid}}
  \index[general]{ibid!disabling use of} Under \oscolashort\ 5, the use of
  `ibid' in citations is abolished by default. If you prefer to
  restore the \oscolashort\ 4 behaviour, you can do so by passing
  \texttt{ibid=true} when loading \biblatex.
\item[\texttt{ibidtracker}] \index[general]{options!ibidtracker@\texttt{ibidtracker}}
  If \texttt{ibid=true} is set, you can disable ibid tracking
  individually using the \biblatex\ option \texttt{ibidtracker=false}.
...
\item[timestamp] For references to audio or video material by time
  code (eg `42:41--51:17'). The postnote will be printed as-is,
  without any prefix.
\end{description}
...
\item[\egcite{[175]\{adams03\}}] \cite[175]{adams03}
...
Under OSCOLA~5, when a case has a Court of Session neutral citation
(CSIH or CSOH), no separate Inner or Outer House designation is
needed: the neutral citation already identifies the
division.\index[general]{cases!Scottish!Inner House}\index[general]{cases!Scottish!Outer House}
Where there is no neutral citation, use the \texttt{institution}
field, set to \texttt{IH} or \texttt{OH}, as in example
\ref{adams03}.
...
\item[ecli]\index[general]{ECLI number}
  The ECLI number for the case report. Under OSCOLA~5, the default
  is to use only the ECLI number. See the options section at
  p~\pageref{options} for details.
...
Under OSCOLA~5, the preferred citation for any ECJ or General Court
decision is the ECLI number (the default behaviour of this package).
The question of how to handle cases not yet reported in the ECR is
therefore largely moot: an ECLI number will be available for
virtually all decisions and is sufficient on its own.

Example \ref{C556/07} shows a case cited via the Official Journal,
which was the approach suggested by earlier editions of
\oscolashort\ for unreported cases. This is retained here as a
historical note. If an ECLI number is available (as it will be for
effectively all modern cases), use that instead and omit the OJ reference.
...
Acts of Parliament (and those of the Scottish Parliament, Welsh
Parliament (Senedd Cymru), and the Northern Ireland Assembly), (b)
secondary legislation (statutory instruments) made in any part of the
UK, (c) draft UK primary legislation (bills), (d) the treaties
establishing the \textsc{eu}, (e) legislation made by the
\textsc{eu}. It does not provide facilities for citing non-UK or
\textsc{eu} legislation. Rules of Court, though technically
legislation, are also dealt with separately: see page
\ref{courtrules}.
...
Primary legislation (that is, UK Acts of Parliament, and acts and
measures of the Scottish Parliament, Welsh Parliament (Senedd Cymru),
National Assembly for Wales (Cynulliad Cenedlaethol Cymru), and the
Northern Ireland Assembly) are cited using the \texttt{@legislation}
...
\index[general]{bibfile@\texttt{.bib} file!number field@\texttt{number} field}
 Give the number of the legislation. This is only
  ...
  legislation. For acts of the Scottish Parliament use \texttt{asp};
  for acts of the Welsh Parliament (Senedd Cymru) use \texttt{asc};
  for acts of the National Assembly for Wales use \texttt{anaw} (in
  English) or \texttt{dccc} (in Welsh); for acts of the Northern
  Ireland Assembly use \texttt{c}.
\item[note]
...
It is also possible to cite Scottish, Welsh and Northern Irish
legislation. Set the \texttt{keywords} field appropriately
(\texttt{sc}, \texttt{cy}, or \texttt{ni}), and include the correct
chapter or session number in the \texttt{number} field.

The following example is a Measure of the National Assembly for Wales
(the predecessor to the Welsh Parliament / Senedd Cymru), which used
\texttt{nawm} numbers.
...
Acts of the National Assembly for Wales use \texttt{anaw} (English
title) or \texttt{dccc} (Welsh title); acts of the Welsh Parliament
(Senedd Cymru) use \texttt{asc}.

\begin{bibexample}[transplantation13]
\begin{verbatim}
@legislation{transplantation13,
  title          = {Deddf Trawsblannu Dynol (Cymru)},
  date           = {2013},
  number         = {dccc 5},
  keywords       = {cy},
  entrysubtype   = {primary},
...
\begin{bibexample}[abolition20]
\begin{verbatim}
@legislation{abolition20,
  title          = {Child (Abolition of Defence of Reasonable
                    Punishment) (Wales) Act},
  date           = {2020},
  number         = {anaw 3},
  keywords       = {cy},
  entrysubtype   = {primary},
...
\begin{description}
\item[\egcite{\{transplantation13\}}] \cite{transplantation13}
\item[\egcite{\{abolition20\}}] \cite{abolition20}
\item[\egcite{\{welshtax22\}}] \cite{welshtax22}
\end{description}
\index[general]{legislation!UK!examples|)}
...
\subsubsection{OJ references without page numbers}

\index[general]{legislation!EU!OJ without page numbers}
More recent issues of the Official Journal do not carry page numbers.
In those circumstances, omit the \texttt{pages} and \texttt{volume}
fields. The citation will give the series letter alone, for example
`[2025] OJ L'.

\begin{bibexample}[eu2025:tariffs]
\begin{verbatim}
@legislation{eu2025:tariffs,
  title        = {Regulation (EU) 2025/783 of the European
                  Parliament and of the Council of 14 April
                  2025 amending Regulation (EU) 2018/196 on
                  additional customs duties on imports of
...
\begin{description}
\item[\egcite{\{eu2025:tariffs\}}] \cite{eu2025:tariffs}
\end{description}

\subsubsection{Assimilated EU law\label{assimilated}}

\index[general]{assimilated EU law|(}
After the United Kingdom's departure from the EU, much directly
applicable EU law was retained in domestic law as `assimilated law'.
Under OSCOLA~5, such instruments are cited with the prefix
`Assimilated', and, where they have been amended, a note of the
amending instrument.

To mark an entry as assimilated, add \texttt{assimilated} to the
\texttt{keywords} field. If the instrument has been amended, put the
amending reference in the \texttt{note} field.\index[general]{keywords field@\texttt{keywords} field!assimilated@\texttt{assimilated}}

\begin{bibexample}[rome1assimilated]
\begin{verbatim}
@legislation{rome1assimilated,
  title        = {Regulation (EC) No 593/2008 of the European
  ...
  note         = {SI 2019/834, reg~10},
}
\end{verbatim}
\end{bibexample}

\begin{description}
\item[\egcite{\{rome1assimilated\}}] \cite{rome1assimilated}
\end{description}
\index[general]{assimilated EU law|)}
...
(the Civil Procedure Rules, the Criminal Procedure Rules, the Family
Procedure Rules, the Rules of the Supreme Court and the County Court
Rules) in a special way.
...
Under OSCOLA~5, the Criminal Procedure Rules and the Family Procedure
Rules are treated in the same way. Like the RSC and CCR, they are
cited by rule number, so \texttt{pagination} is set to
\texttt{rule}:

\begin{bibexample}[crpr2020]
\begin{verbatim}
@legislation{crpr2020,
...
Note that strictly speaking the \texttt{title} field is not essential;
it will not be printed in citations (but will be used simply for
indexing).
...
\item[\egcite{[r~17]\{crpr2020\}}] \cite[r~17]{crpr2020}
\item[\egcite{[r~4.1]\{fpr2010\}}] \cite[r~4.1]{fpr2010}
\end{description}
...
Where an article has appeared in a newspaper, slightly different styling is required. In those circumstances, use the \verb|entrysubtype| of \verb|newspaper|. Under OSCOLA~5, the city of publication is no longer required, so the \texttt{location} field may be omitted.
...
\subsection{Book Reviews\label{bookreviews}}

\index[general]{book reviews}
\index[general]{articles!entrysubtype field@\texttt{entrysubtype} field!bookreview@\texttt{bookreview}}
Book reviews should be entered as \texttt{@article} with the
\texttt{entrysubtype} set to \texttt{bookreview}. Under OSCOLA~5, the
annotation `(book review)' will be appended automatically.

\begin{bibexample}[guest:1961]
\begin{verbatim}
@article{guest:1961,
  entrysubtype = {bookreview},
  author       = {Guest, A G},
...
\begin{description}
\item[\egcite{\{guest:1961\}}] \cite{guest:1961}
\item[\egcite{\{nolan:2012\}}] \cite{nolan:2012}
\end{description}
...
Under OSCOLA~5, thesis titles are italicised rather than placed in
quotation marks. This is handled automatically.
...
\subsubsection{Perma.cc URLs}

\index[general]{Perma.cc}
\index[general]{urldate field@\texttt{urldate} field!Perma.cc suppression}
Under OSCOLA~5, no access date is needed for permanently archived
URLs. Where the \texttt{url} field begins with \texttt{perma.cc},
the `accessed' date will be suppressed automatically, even if a
\texttt{urldate} is present in the entry.

\begin{bibexample}[cyclefree:2023]
\begin{verbatim}
@online{cyclefree:2023,
  author       = {{Cyclefree}},
  title        = {Is This Really Necessary, Minister?},
  journaltitle = {Legal Feminist},
  date         = {2023-04-27},
...
\begin{description}
\item[\egcite{\{cyclefree:2023\}}] \cite{cyclefree:2023}
\end{description}

\subsubsection{Podcasts}

\index[general]{podcast}
\index[general]{entrysubtype field@\texttt{entrysubtype} field!podcast@\texttt{podcast}}
Podcasts should be entered as \texttt{@online} with the
\texttt{entrysubtype} set to \texttt{podcast}. Where there is a
named author or host, use the \texttt{author} field; where there is
no personal author, put the series name in the \texttt{author} field
(using double braces to prevent name reordering). Use
\texttt{journaltitle} for the series title if a personal author is
present. Timestamp pinpoints are passed as postnotes when citing
(set \texttt{pagination} to \texttt{timestamp} in the entry so they
are printed as-is).

\begin{bibexample}[doublejeopardy:ep27]
\begin{verbatim}
@online{doublejeopardy:ep27,
  entrysubtype = {podcast},
  author       = {{Double Jeopardy podcast}},
  title        = {Episode 27: Dr Bryn Harris -- Free Speech,
                  Harm and the Internet},
  date         = {2023-04-07},
  url          = {https://podcasts.apple.com/gb/podcast/
                  double-jeopardy/id1633485236?i=1000607857832},
  urldate      = {2023-07-21},
...
\begin{description}
\item[\egcite{[3:40--3:56]\{doublejeopardy:ep27\}}] \cite[3:40--3:56]{doublejeopardy:ep27}
\end{description}

\subsubsection{Social media posts}

\index[general]{social media}
\index[general]{entrysubtype field@\texttt{entrysubtype} field!socialmedia@\texttt{socialmedia}}
Social media posts should be entered as \texttt{@online} with the
\texttt{entrysubtype} set to \texttt{socialmedia}. Use
\texttt{journaltitle} for the platform name and \texttt{note} for
the time of posting. Where the author is identified only by an
account handle, put it in the \texttt{author} field using double
braces to prevent name reordering.

\begin{bibexample}[criminalbar:tweet23]
\begin{verbatim}
@online{criminalbar:tweet23,
  entrysubtype = {socialmedia},
  author       = {{@The Criminal Bar}},
...
\begin{description}
\item[\egcite{\{criminalbar:tweet23\}}] \cite{criminalbar:tweet23}
\end{description}

\subsubsection{Audio and video: timestamp pinpoints}

\index[general]{timestamp pinpoints}
\index[general]{pagination field@\texttt{pagination} field!timestamp@\texttt{timestamp}}
For audio or video material where pinpoint references are to a time
code rather than a page, set \texttt{pagination} to
\texttt{timestamp}. The postnote will then be printed as-is (without
any `p.'\ or `para'\ prefix), placed between the date parenthesis
and the \textsc{url}. Note that only postnotes on entries with
\texttt{pagination=timestamp} are treated this way; text postnotes
on other entries continue to appear at the end of the citation as
usual.

\begin{bibexample}[uksc:hale19]
\begin{verbatim}
@online{uksc:hale19,
  author       = {{UK Supreme Court}},
  title        = {Lady Hale's Valedictory Remarks --
                  18 December 2019},
...
\begin{description}
\item[\egcite{[42:41--51:17]\{uksc:hale19\}}] \cite[42:41--51:17]{uksc:hale19}
\end{description}

\subsubsection{Generative artificial intelligence}

\index[general]{generative artificial intelligence}
\index[general]{entrysubtype field@\texttt{entrysubtype} field!ai@\texttt{ai}}
Under OSCOLA~5, generative AI outputs are cited with the AI model as
the author. This is a contentious choice, and the material, by definition, is not stable and cannot be looked up by a reader. If you have provided instructions to generate the material, OSCOLA~5 says you may 
specify those instructions, which can be done in the \texttt{title} field, which will be
printed in inverted commas preceded by `response to'. Give the name of
the organisation in the \texttt{publisher} field and the date on which
the material was generated in the \texttt{date} field.

Use the \texttt{@online} entry type with \texttt{entrysubtype} set to
\texttt{ai}. No URL is needed; the format does not include one --- presumably due to lack of stability. You may never need this macro but it is here for completeness.

\begin{bibexample}[chatgpt:ai23a]
\begin{verbatim}
@online{chatgpt:ai23a,
  entrysubtype = {ai},
  author       = {{ChatGPT}},
  title        = {Explain how artificial intelligence works},
  publisher    = {OpenAI},
  date         = {2023-07-16},
}
\end{verbatim}
...
\begin{bibexample}[chatgpt:ai23b]
\begin{verbatim}
@online{chatgpt:ai23b,
  entrysubtype = {ai},
  author       = {{ChatGPT}},
  title        = {How to cite ChatGPT in the style of OSCOLA},
  publisher    = {OpenAI},
  date         = {2023-07-16},
}
\end{verbatim}
...
\begin{description}
\item[\egcite{\{chatgpt:ai23a\}}] \cite{chatgpt:ai23a}
\item[\egcite{\{chatgpt:ai23b\}}] \cite{chatgpt:ai23b}
\end{description}
